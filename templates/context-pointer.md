...
_Generated by the Number Pii toolkit._
...
8. **Writing Standards**: prose deliverables you write follow the toolkit's `WRITING.md`
   (no em or en dashes, no banned phrases, human-quality prose); validate with
   `python3 organisation/scripts/check_writing.py <file>` before handover. The standard
   covers only what you author: never restyle lines you are not otherwise changing,
   generated or vendored files, or blocks managed by other tools (such as
   `<!-- BEGIN:nextjs-agent-rules -->` sections); commit a tool's own updates unchanged.
   Code comments follow the codebase's existing conventions.
