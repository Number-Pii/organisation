...
├── scripts/                                   # Scaffolder, handover, docs, skills, release tooling
│   ├── init_project.py                        # Project scaffolder (AGENTS.md, doc/, hooks)
│   ├── handover.py                            # Per-branch handover entries and consolidation
│   ├── docs.py                                # Documentation map and orphan/link check
│   ├── sync_context.py                        # Refreshes the toolkit block in a project's AGENTS.md
│   ├── lib/                                   # Shared helpers (frontmatter, runners, file writes)
│   └── README.md                              # Every script, one section each
├── templates/                                 # Scaffold templates rendered by init_project.py
├── tests/                                     # Pytest suite (run in CI; see tests/README.md)
├── agents/                                    # Core role subagents (generated by build_agents.py)
├── evals/                                     # Scenario and skill evals (see evals/README.md)
├── changes/                                   # Changelog fragments, one per PR
├── .claude-plugin/ + commands/ + hooks/       # Claude Code plugin packaging (/np:init, hooks)
├── .githooks/pre-commit                       # Local checks; install: git config core.hooksPath .githooks
├── AGENTS.md                                  # Instructions for agents maintaining this repo
├── CLAUDE.md, GEMINI.md                       # Stubs that import AGENTS.md
├── INITIALIZE.md                              # Initialize Protocol (read on demand)
├── WRITING.md                                 # Writing Standard (read on demand for prose work)
├── GITHUB_ORCHESTRATION.md                    # Task board rules (read on demand)
├── CONTRIBUTING.md                            # How to change the toolkit
...
The repo doubles as a Claude Code plugin (manifest in `.claude-plugin/`). Installed as a plugin, the Initialize Protocol becomes the `/np:init` command, the main-branch protection hook runs everywhere the plugin is enabled, and the core role agents (`agents/np-*.md`) are invokable directly. Install from a local clone:
```
/plugin install <path-to-this-repo>
```

### Initialize (any coding agent)
Clone this repo into your project folder (see below), then ask your AI assistant to
**initialize** the project, or run `/np:init` in Claude Code. The assistant follows
`INITIALIZE.md`: it agrees the brief, classification level, and team with you, then runs
`scripts/init_project.py`, which writes:
- a root `AGENTS.md` holding your project description and the toolkit's managed block
  (boundaries, where to look, PR checklist, handover format), with `CLAUDE.md` and
  `GEMINI.md` stubs that import it
- the `doc/` tree with its map (`doc/README.md`), brief, workflow, version control, task
  board, and per-branch handover (`doc/handover/entries/`, `doc/handover/STATE.md`)
- a git `pre-push` hook that refuses pushes to `main`, and an optional Claude Code hook
...
`AGENTS.md` is the one instruction file. OpenAI Codex, Cursor, GitHub Copilot and most other
agents read it directly; Claude Code and Gemini CLI reach it through the `@AGENTS.md` import
in their stubs. Everything an agent needs to work safely is inside the project's own
`AGENTS.md`, so cloud sessions that clone the project without the toolkit still have it.
The scenario evals (`evals/`) run against both Claude Code and Codex.

The standards every project inherits live in the managed block,
[templates/agents-block.md](templates/agents-block.md). After updating the toolkit, refresh a
project's copy with `python3 organisation/scripts/sync_context.py`.
...
> Maintained by Number Pii leadership. The toolkit version is in [VERSION](VERSION).
