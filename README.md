...
Number Pii hires only the top 1% in each field. Ring-fenced product engineering, elite services, expert-led consultancy, security by design.

For the full philosophy, org chart, delegation model, and approval authority matrix, see [Teams/organisation.md](Teams/organisation.md) and [Teams/philosophy.md](Teams/philosophy.md).
...
### Initialize (Claude, Gemini, or Codex)
...
- `CLAUDE.md` is the source of truth; `GEMINI.md` and `AGENTS.md` are generated by `scripts/sync_ai_context.py` (see [CONTRIBUTING.md](CONTRIBUTING.md#keeping-claudemd-geminimd-and-agentsmd-in-sync))
- All three support the same `initialize` command and follow the same protocol
